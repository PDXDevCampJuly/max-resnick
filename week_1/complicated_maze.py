...
def room0():
    ...
    doors = {"north": room1,
             "west": death,
             "south": room2}
    inventory = {room3: {'west': room4}}
    return (description, doors, inventory)
...
def process_movement(start_position):
    inventory = {}
    description, doors, found_inventory = start_position()
    while choice.lower() != 'leave':
        ...
        # check if we use a door or exit.
        if choice.lower() == 'search':
            inventory.update(search(room_data))
        if doors in inventory:
            doors.update(inventory[doors])
        # Get room
        doors[choice.lower()]()

if __name__ == '__main__':
    print("Welcome to the Alice in Wonderland Maze!")
    process_movement(room0)
